...
LOCKFILE := $(HOME)/.dotapps/dev.lock
...
kill-dev:
	...
	@sleep 1
	@-rm -f ~/.dotapps/vm/alpine.img ~/.dotapps/vm/vfkit-efi-store 2>/dev/null || true
	@-rm -f ~/.dotapps/vm/agent.sock ~/.dotapps/vm/gvproxy-*.sock ~/.dotapps/vm/vfkit-*.sock 2>/dev/null || true
...
reset-state:
	@python3 -c "import json; f=open('$(HOME)/.dotapps/state.json'); s=json.load(f); f.close(); [p.update({'status':'stopped','intent':'stop','port':None}) for p in s['projects']]; s['nextPort']=3001; f=open('$(HOME)/.dotapps/state.json','w'); json.dump(s,f,indent=2); f.close(); print('State reset: all projects stopped')"
...
status:
	...
	@echo "=== Agent ==="
	@test -S ~/.dotapps/vm/agent.sock && echo "  UP" || echo "  DOWN"
	@echo "=== Vite ==="
	@curl -s -o /dev/null -w "  HTTP %{http_code}\n" http://localhost:1420/ 2>/dev/null || echo "  DOWN"
	@echo "=== Projects ==="
	@python3 -c "import json; f=open('$(HOME)/.dotapps/state.json'); s=json.load(f); [print(f'  {p[\"name\"]}: {p[\"status\"]} intent={p.get(\"intent\")} port={p.get(\"port\")}') for p in s['projects']]" 2>/dev/null || echo "  no state file"
...
help:
	...
	@echo "  make clean-all    - Clean everything (including ~/.dotapps)"
	@echo ""
	@echo "Quick Start:"
